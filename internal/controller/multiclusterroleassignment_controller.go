...
func (r *MulticlusterRoleAssignmentReconciler) validateSpec(ctx context.Context, mra *rbacv1alpha1.MulticlusterRoleAssignment) error {
...
// validateClusterSets validates that all referenced ManagedClusterSets exist and updates role assignment statuses accordingly.
func (r *MulticlusterRoleAssignmentReconciler) validateClusterSets(ctx context.Context, mra *rbacv1alpha1.MulticlusterRoleAssignment) error {
	...
	for _, roleAssignment := range mra.Spec.RoleAssignments {
		...
		for _, clusterSetName := range roleAssignment.ClusterSets {
			...
			if err != nil {
				if apierrors.IsNotFound(err) {
					log.Info("Referenced ManagedClusterSet not found", "clusterSet", clusterSetName, "roleAssignment", roleAssignment.Name)
					missingClusterSetsForRA = append(missingClusterSetsForRA, clusterSetName)
					allMissingClusterSets = append(allMissingClusterSets, clusterSetName)
				} else {
					log.Error(err, "Failed to get ManagedClusterSet", "clusterSet", clusterSetName, "roleAssignment", roleAssignment.Name)
					return fmt.Errorf("failed to validate cluster set %s: %w", clusterSetName, err)
				}
			}
		}
		...
		if len(missingClusterSetsForRA) > 0 {
			r.setRoleAssignmentStatus(mra, roleAssignment.Name, StateTypeFailed,
				fmt.Sprintf("Missing ManagedClusterSets: %v", missingClusterSetsForRA))
		} else {
			r.setRoleAssignmentStatus(mra, roleAssignment.Name, StateTypePending,
				"ManagedClusterSet validation passed")
		}
	}
