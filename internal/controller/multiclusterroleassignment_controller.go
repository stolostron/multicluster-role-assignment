...
func (r *MulticlusterRoleAssignmentReconciler) checkClusterRoleBindingStatus(
	...
	valErrs, valUnknowns := checkClusterRoleValidation(mra, raSpec, cluster, bindingsMap)
...
// isValidationEnabled reports whether this MRA opted into ClusterRole existence checks.
func isValidationEnabled(mra *mrav1beta1.MulticlusterRoleAssignment) bool {
	return mra != nil && mra.Spec.Validate != nil && *mra.Spec.Validate
}

// checkClusterRoleValidation reports errors or pending messages from ClusterPermission
// ValidateClusterRolesExist when this MRA has spec.validate=true. Failures are attributed
// only when this assignment's ClusterRole is listed in the condition message, because
// multiple MRAs share one ClusterPermission.
...
// applyClusterPermissionValidate sets ClusterPermission.spec.validate. This MRA opting in
// forces true. Otherwise the existing shared ClusterPermission value is preserved so a
// non-validating MRA cannot disable validation enabled by another owner.
func applyClusterPermissionValidate(newSpec *cpv1alpha1.ClusterPermissionSpec,
	mra *mrav1beta1.MulticlusterRoleAssignment, existing *cpv1alpha1.ClusterPermission) {
	if isValidationEnabled(mra) {
		enabled := true
	...
	if existing != nil {
		newSpec.Validate = existing.Spec.Validate
	}
}
...
func (r *MulticlusterRoleAssignmentReconciler) ensureClusterPermissionAttempt(ctx context.Context,
	...
	if existingCP == nil {
		...
		applyClusterPermissionValidate(&newSpec, mra, nil)
	...
	applyClusterPermissionValidate(&newSpec, mra, existingCP)
